...
import { Crown, CheckCircle2, ShieldCheck, Zap, LockKeyhole, Loader2, Link2, Coins } from 'lucide-react';
...
export default function VIPPlan() {
  ...
  const initiateTransaction = async () => {
    ...
    try {
      ...
      if (res.ok && data.success && data.invoiceUrl) {
        toast('Invoice established. Redirecting to Apirone secure gateway.', 'success');
  ...
  return (
    <div className="max-w-5xl mx-auto md:py-8 animation-fade-in">
      <SeoHead title="Elite Protocol Access" description="Secure lifetime VIP access utilizing decentralized blockchain networks." />
      
      <div className="text-center mb-12">
        <div className="inline-flex items-center justify-center p-3 bg-orange-500/10 rounded-2xl border border-orange-500/20 mb-6 shadow-sm">
      ...
      <div className="grid md:grid-cols-2 gap-8 items-start bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="space-y-8 z-10 pt-4">
          <div>
             <h2 className="text-2xl font-bold text-zinc-900 dark:text-white mb-2">Protocol Architecture</h2>
             <p className="text-zinc-500 text-sm">Validating credentials against the central registry.</p>
          </div>
          <ul className="space-y-4">
            {[
              { icon: LockKeyhole, text: 'Decrypted read access to locked data streams.' },
              { icon: Zap, text: 'Permanent bypass of point expenditure parameters.' },
              { icon: ShieldCheck, text: 'Cryptographic VIP verification badge globally active.' },
              { icon: CheckCircle2, text: 'Priority routing and network optimization.' }
            ].map((item, idx) => (
        ...
        <div className="bg-zinc-50 dark:bg-[#0a0a0a] rounded-2xl p-6 border border-zinc-200 dark:border-zinc-800 text-center z-10 flex flex-col gap-6 shadow-inner">
          ...
          <div className="flex flex-col items-center justify-center">
            ...
            <span className="text-xs text-zinc-500 mt-1 font-medium">One-Time Fiat Equivalent Integration</span>
          </div>

          <div className="space-y-3 text-left w-full">
             <label className="text-sm font-bold text-zinc-700 dark:text-zinc-300 flex items-center gap-2">
               <Coins className="size-4" /> Destination Network
             </label>
               ...
               {SUPPORTED_NETWORKS.map(net => (
                 ...
                 >
                   <div className="flex items-center justify-center gap-2">
                     {net.symbol} <span className="text-[10px] uppercase opacity-60">({net.name})</span>
                   </div>
                 </button>
               ))}
             </div>
          ...
          {user?.isVip ? (
            <div className="w-full bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 font-bold py-4 rounded-xl flex items-center justify-center gap-2 shadow-sm">
              <CheckCircle2 className="size-5" /> VIP Protocol Active
            </div>
          ) : (
            <button 
            ...
            >
              {isProcessing ? <Loader2 className="size-5 animate-spin" /> : <Link2 className="size-5" />}
              {isProcessing ? 'Compiling Invoice...' : 'Deploy via Apirone'}
            </button>
          )}
          <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Secured By Apirone Cryptography</p>
        </div>
      </div>
    </div>
  );
