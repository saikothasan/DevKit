...
const ReplyCard = memo(({ 
  ...
  return (
    <article className={`relative bg-[var(--surface)] border rounded-[1.5rem] md:rounded-[2rem] p-5 md:p-8 ml-0 md:ml-12 flex flex-col sm:flex-row gap-5 md:gap-8 transition-all hover:shadow-xl shadow-zinc-200/20 dark:shadow-black/20 ${reply.isAcceptedAnswer ? 'border-emerald-500/50 bg-emerald-500/5' : reply.authorIsVip ? 'border-amber-500/30' : 'border-[var(--border)] hover:border-[var(--orange-border)]'}`}>
      ...
      {(isModerator || isAuthor) && (
        <button onClick={() => onDelete(reply.id)} className="absolute top-4 right-4 md:top-6 md:right-6 p-2 text-zinc-500 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all cursor-pointer shadow-sm" title="Wipe Reply">
      ...
      <aside className="flex sm:flex-col items-center justify-between sm:justify-start gap-4 sm:gap-2 shrink-0 border-b sm:border-b-0 sm:border-r border-[var(--border)] pb-4 sm:pb-0 sm:pr-6 md:pr-8">
        <div className="flex sm:flex-col items-center gap-3 sm:gap-2">
          <button 
            ...
            disabled={isAuthor}
            className={`p-2.5 md:p-3 rounded-xl md:rounded-2xl transition-all border border-transparent shadow-sm group ${isAuthor ? 'text-zinc-500 bg-[var(--surface-raised)] opacity-50 cursor-not-allowed' : 'text-zinc-500 hover:text-[var(--orange)] bg-[var(--surface-raised)] hover:border-[var(--orange-border)] cursor-pointer hover:-translate-y-1 hover:shadow-md'}`}
          >
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="prose prose-zinc dark:prose-invert max-w-none prose-p:leading-relaxed text-sm md:text-base prose-pre:bg-[#0a0a0a] prose-pre:border prose-pre:border-[var(--border-strong)] prose-pre:rounded-xl break-words overflow-x-auto">
...
export default function Thread() {
  ...
  return (
    <article className="w-full max-w-5xl mx-auto py-6 md:py-8 px-4 sm:px-6 animation-fade-in overflow-x-hidden">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6 mb-6 md:mb-8 border-b border-[var(--border)] pb-4 md:pb-6">
        ...
        {(isModerator || isAuthor) && (
          <div className="flex items-center justify-center sm:justify-end gap-2 bg-[var(--surface-raised)] p-1.5 rounded-xl border border-[var(--border)] shadow-inner w-full sm:w-auto">
            {isModerator && (
              <>
                <button onClick={() => handleModeration('pin')} className={`flex-1 sm:flex-none flex justify-center p-2 md:p-2.5 rounded-lg transition-all cursor-pointer ${thread.isPinned ? 'bg-orange-500 text-white shadow-md' : 'text-zinc-500 hover:text-[var(--text-primary)] hover:bg-[var(--surface)] hover:shadow-sm'}`} title="Pin Thread">
                  <Pin className="size-4" />
                </button>
                <button onClick={() => handleModeration('lock')} className={`flex-1 sm:flex-none flex justify-center p-2 md:p-2.5 rounded-lg transition-all cursor-pointer ${thread.isLocked ? 'bg-red-500 text-white shadow-md' : 'text-zinc-500 hover:text-[var(--text-primary)] hover:bg-[var(--surface)] hover:shadow-sm'}`} title="Lock Thread">
                  <LockKeyhole className="size-4" />
                </button>
              </>
            )}
            <button onClick={() => handleModeration('delete')} className="flex-1 sm:flex-none flex justify-center p-2 md:p-2.5 rounded-lg text-zinc-500 hover:text-red-500 hover:bg-red-500/10 transition-all sm:ml-1 cursor-pointer" title="Wipe Vector">
              <Trash2 className="size-4" />
            </button>
          </div>
        )}
      ...
      {/* Main Thread Body */}
      <section className={`bg-[var(--surface)] backdrop-blur-xl border rounded-[1.5rem] md:rounded-[2rem] p-5 sm:p-6 md:p-10 shadow-xl shadow-zinc-200/20 dark:shadow-black/40 mb-8 md:mb-12 flex flex-col sm:flex-row gap-5 md:gap-10 ${thread.isPinned ? 'border-orange-500/50 bg-gradient-to-br from-[var(--orange-dim)] to-transparent' : thread.authorIsVip ? 'border-amber-500/40 shadow-amber-500/5' : 'border-[var(--border)]'}`}>
        ...
        <div className="flex-1 min-w-0 w-full overflow-hidden">
          <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-4 md:mb-6">
             ...
             <div className="flex sm:hidden items-center gap-1.5 bg-[var(--surface-raised)] border border-[var(--border)] px-2 py-1 rounded-md ml-auto shadow-sm">
                <button onClick={() => handleVote('thread', thread.id, thread.authorId)} disabled={user?.id === thread.authorId} className={`p-0.5 rounded transition-all ${user?.id === thread.authorId ? 'text-zinc-500 opacity-50' : 'text-zinc-500 hover:text-[var(--orange)] active:scale-95'}`}>
          ...
          <h1 className="text-2xl sm:text-3xl md:text-5xl font-black mb-6 md:mb-8 text-balance leading-tight text-[var(--text-primary)] tracking-tight break-words">{thread.title}</h1>
          
          <div className="prose prose-zinc dark:prose-invert max-w-none prose-p:leading-relaxed text-sm md:text-lg prose-headings:font-bold prose-a:text-[var(--orange)] hover:prose-a:text-orange-600 prose-pre:bg-[#0a0a0a] prose-pre:border prose-pre:border-[var(--border-strong)] prose-pre:rounded-xl md:prose-pre:rounded-2xl prose-pre:shadow-inner prose-code:text-[var(--orange)] prose-code:bg-[var(--orange-dim)] prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded-md prose-code:before:content-none prose-code:after:content-none break-words overflow-x-auto">
              ...
              {thread.lockedContent ? (
                <div className="animation-fade-in pl-2 md:pl-0">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 md:gap-4 mb-4 md:mb-6">
                      ...
                      {(user?.isVip || user?.role === 'admin') && user?.id !== thread.authorId && (
                         <span className="flex items-center self-start sm:self-auto gap-1 md:gap-1.5 px-2.5 md:px-3 py-1 bg-amber-500/10 border border-amber-500/20 text-amber-500 text-[9px] md:text-[10px] font-bold uppercase tracking-wider rounded-md shadow-sm">
                           <Crown className="size-2.5 md:size-3" /> VIP Override
                         </span>
                      )}
                  </div>
                  <div className="bg-[#0a0a0a] p-4 md:p-6 rounded-xl md:rounded-2xl border border-[var(--border-strong)] overflow-x-auto shadow-inner">
                ...
                <div className="flex flex-col lg:flex-row items-center justify-between gap-6 md:gap-8 pl-2 md:pl-0">
                  ...
                  {user ? (
                    ...
                    <Link to="/login" className="shrink-0 w-full lg:w-auto flex items-center justify-center gap-2 bg-[var(--orange)] hover:opacity-90 text-[#fff] font-bold px-6 md:px-10 py-4 md:py-5 rounded-xl md:rounded-2xl transition-all shadow-xl shadow-orange-500/20 active:scale-[0.98] text-base md:text-lg">
      ...
      {thread.isLocked && !isModerator ? (
        ...
        <section className="bg-[var(--surface)] backdrop-blur-xl border border-[var(--border)] rounded-[1.5rem] md:rounded-[2rem] p-5 sm:p-6 md:p-10 shadow-2xl shadow-zinc-200/20 dark:shadow-black/40 relative overflow-hidden mx-0 md:ml-12">
          <div className="absolute top-0 left-0 right-0 h-1 md:h-1.5 bg-gradient-to-r from-orange-500 to-amber-400"></div>
          ...
          {user ? (
            ...
            <div className="flex flex-col items-center justify-center p-6 md:p-10 bg-[var(--surface-raised)] rounded-2xl border border-[var(--border)] shadow-inner text-center">
               ...
               <Link to="/login" className="w-full sm:w-auto px-8 md:px-10 py-3.5 md:py-4 bg-[var(--orange)] hover:opacity-90 text-[#fff] rounded-xl md:rounded-2xl font-bold transition-all shadow-xl shadow-orange-500/20 active:scale-[0.98] text-base md:text-lg">Authenticate Entity</Link>
            </div>
          )}
        </section>
      )}
